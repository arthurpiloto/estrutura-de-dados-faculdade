...
void decrescente_ordem(NO* raiz, int nivel){
    if(raiz != NULL){
        decrescente_ordem(raiz->dir, nivel+1);
        printf("[%d, %d] ", raiz->info, nivel);
        decrescente_ordem(raiz->esq, nivel+1);
    }
}
...
void imprime(ABP* raiz){
    ...
    printf("\nEm Ordem Decrescente: "); decrescente_ordem(*raiz, 0);
    printf("\n");
}
